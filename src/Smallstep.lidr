...
Here is the same evaluator, written in exactly the same style, 
but formulated as an inductively defined relation. Again, we use 
the notation `t >>> n` for "`t` evaluates to `n`."
...
Right-hand sides of sums can take a step only when the left-hand side
is finished: if `t2` can take a step to `t2'`, then `P (C n) t2` steps
to `P (C n) t2'`:
...
> deterministic {xt} r = {x : xt} -> {y1 : xt} -> {y2 : xt} -> r x y1 -> r x y2 -> y1 = y2
>
> step_deterministic : deterministic Step'
> step_deterministic ST_PlusConstConst' ST_PlusConstConst' = Refl
> step_deterministic ST_PlusConstConst' (ST_Plus1' _) impossible
> step_deterministic ST_PlusConstConst' (ST_Plus2' _) impossible
> step_deterministic (ST_Plus1' l) ST_PlusConstConst' impossible
> step_deterministic (ST_Plus1' l) (ST_Plus1' l') = rewrite step_deterministic l l' in Refl
> step_deterministic (ST_Plus1' l) (ST_Plus2' _) impossible
> step_deterministic (ST_Plus2' r) ST_PlusConstConst' impossible
> step_deterministic (ST_Plus2' r) (ST_Plus1' _) impossible
> step_deterministic (ST_Plus2' r) (ST_Plus2' r') = rewrite step_deterministic r r' in Refl
>

\todo[inline]{Matching on implicit shortens the proof}

> step_deterministic_alt : deterministic Step' 
> step_deterministic_alt {x=P (C _) (C _)}  ST_PlusConstConst' ST_PlusConstConst' = Refl
> step_deterministic_alt {x=P  _     _}    (ST_Plus1' s1)      (ST_Plus1' s2)     = 
>   rewrite step_deterministic s1 s2 in Refl
> step_deterministic_alt {x=P (C _)  _}    (ST_Plus2' s1)      (ST_Plus1' s2)     = absurd s2
> step_deterministic_alt {x=P (C _)  _}    (ST_Plus2' s1)      (ST_Plus2' s2)     = 
>   rewrite step_deterministic s1 s2 in Refl
>
...
to maintain a close correspondence between the informal and Idris
...
to terms that cannot make progress.  We'll call them _normal forms_.
...
terminology for talking about other relations later in the course.
...
> multi_R : {t : Type} -> {r : Relation t} -> (x, y : t) -> r x y -> (Multi r) x y
